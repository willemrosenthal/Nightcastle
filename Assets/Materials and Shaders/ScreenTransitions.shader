...
Shader "Hidden/ScreenTransitions"
{
	Properties
	{
		...
		_WhiteTransition("White Transition", float) = 0
		...
		SubShader
		{
			...
			Pass
			{
				...
				{
					fixed4 transit = tex2D(_TransitionTex, (i.uv1 + float2 (_XPos, _YPos)) * 0.5);
					...
					fixed4 fadeColor = fixed4(1 * _WhiteTransition, 1 * _WhiteTransition, 1 * _WhiteTransition, 1);

					// normal fade
					col = lerp(col, fadeColor, _Fade);
